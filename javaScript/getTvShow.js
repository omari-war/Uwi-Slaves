function displayTvShowTitleData(records) {
    let html = '';
...
async function getTvShowTitleData() {
    const response = await fetch(`https://moviesdatabase.p.rapidapi.com/titles?page=${pageNumber}&limit=${limit}&list=most_pop_series&sort=year.decr`, options);
    if (!response.ok) {
